...
execute if block ~ ~2 ~ #game:move_through run execute unless entity @e[type=#game:hostile, distance=0..2] positioned ^ ^ ^1 run execute if entity @s[distance=..100] run function game:redstone_blaster_raycast